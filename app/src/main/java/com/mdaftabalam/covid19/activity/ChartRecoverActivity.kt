 ...
 **/
class ChartRecoverActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_chart_recover)
        x = ArrayList()
        y = ArrayList()
        mChart = findViewById(R.id.graph_recover)
        mChart.setDrawGridBackground(false)
        mChart.setDescription("Recover")
            ...
            Request.Method.GET, allAPI, Response.Listener { response ->
                ...
                try {
                    ...
                    for (i in 0 until jsonArray.length()) {
                        ...
                        val case = objectCases.getString("dailyrecovered")
                        x.add(Entry(case.toInt().toFloat(), i))
                        y.add(objectCases.getString("date"))
                    }
                    val set1 = LineDataSet(x, "Daily Recovered")
