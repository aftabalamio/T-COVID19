 ...
 **/
class ChartConfirmActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_chart_confirm)
        x = ArrayList()
        y = ArrayList()
        mChart = findViewById(R.id.graph_confirm)
        mChart.setDrawGridBackground(false)
        mChart.setDescription("Cases")
            ...
            Request.Method.GET, allAPI, Response.Listener { response ->
                ...
                try {
                    ...
                    for (i in 0 until jsonArray.length()) {
                        ...
                        val case = objectCases.getString("dailyconfirmed")
                        x.add(Entry(case.toInt().toFloat(), i))
                        y.add(objectCases.getString("date"))
                    }
                    val set1 = LineDataSet(x, "Daily Confirmed")
